...
b);c.innerHTML=a};
window.onkeypress=function(a){a=a||window.event;a=a.which||a.keyCode;13===a?toggleHandler(whoIsActive()):a===keyAll&&toggleSame();if(a===keyNext)moveActive(visible,!0,1);else if(a===keyPrev)moveActive(visible,!1,1);else if(a===keyFirst)clearActive(visible),makeActive(visible[0]);else if(a===keyLast)clearActive(visible),makeActive(visible[visible.length-1]);else if(a===keyPrevUp)nextUp(!1);else if(a===keyNextUp)nextUp(!0);else if(a===keyExpand)for(;isAnythingHidden(visible);)for(a=0;a<visible.length;a++)visible[a].classList.contains("collapsed")&&
...
function clearActive(a){for(var b=0;b<a.length;b++)a[b].classList.remove("active")}function getElements(a,b,c){for(var d=0,f=[],e=0;e<elements.length;e++)if(!elements[e].parentElement.classList.contains("js-infobox")&&!elements[e].classList.contains("js-infobox")&&(a&&isOneOf(elements[e].tagName,b)||!a&&!isOneOf(elements[e].tagName,b)))if(!c||c&&!elements[e].classList.contains("hidden"))f[d]=elements[e],d++;return f}
function yesHeaderTag(a){a=a.tagName;return"H1"===a||"H2"===a||"H3"===a||"H4"===a||"H5"===a||"H6"===a}function isOneOf(a,b){for(var c=0;c<b.length;c++)if(a===b[c])return!0};
