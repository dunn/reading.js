...
function toggleHandler(a){var b;isClick(a)?(clearActive(visible),yesHeaderTag(a.target.tagName)?(b=a.target,b.classList.add("active")):a.target.classList.contains("all")?(b=document.getElementsByTagName(a.target.parentElement.tagName),a.target.parentElement.classList.add("active")):(b=a.target.parentElement,b.classList.add("active"))):b=a;void 0===b.length&&yesHeaderTag(b.tagName)?toggleMe(b):toggleSame();a=whoIsActive();makeActive(a);visible=getElements(!1,["HEADER"],!0)}
function toggleMe(a){for(var b=0;b<elements.length;b++)if(elements[b]===a){var c=getHeaderNum(a),c=compareHeaders(b,elements,c);if(isCollapsed(a)){toggleCollapse(a);for(b+=1;elements[b]!==c&&void 0!==elements[b];)if(yesHeaderTag(elements[b].tagName))if(isCollapsed(elements[b])){var d=getHeaderNum(elements[b]),d=compareHeaders(b,elements,d);for(elements[b].classList.remove("hidden");elements[b]!==d&&void 0!==elements[b];)b++}else elements[b].classList.remove("hidden"),b++;else elements[b].classList.remove("hidden"),
...
f===b?toggleHandler(visible[e]):d++:isCollapsed(visible[e])&&f===b?toggleHandler(visible[e]):d++;e--}for(c=c-d+1;getHeaderNum(visible[c])>=b;)d=getHeaderNum(visible[c]),isCollapsed(a)?!isCollapsed(visible[c])&&d===b&&toggleHandler(visible[c]):isCollapsed(visible[c])&&d===b&&toggleHandler(visible[c]),c++;break}}}function compareHeaders(a,b,c){for(a+=1;a<b.length;a++){if(a===b.length-1)return elements[elements.length];if(yesHeaderTag(b[a].tagName)&&b[a].tagName.slice(1)<=c)return b[a]}}
...
function nextUp(a){for(var b=whoIsActive(),c=0;c<visible.length;c++)if(visible[c]===b){for(c=a?c+1:c-1;a?c<visible.length:0<=c;a?c++:c--)if(yesHeaderTag(visible[c].tagName)&&(getHeaderNum(b)>getHeaderNum(visible[c])||!yesHeaderTag(b.tagName))){clearActive(visible);makeActive(visible[c]);break}break}}function makeActive(a){a.classList.add("active");var b=a.getAttribute("id");null!==b&&window.location.replace(url+"#"+b);a.scrollIntoView()}
...
function yesHeaderTag(a){return"H1"===a||"H2"===a||"H3"===a||"H4"===a||"H5"===a||"H6"===a};
